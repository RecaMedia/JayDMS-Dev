...
const SortableItem = SortableElement(({itemIndex, fields, item, updateCallback, removeItem}) => {
  return <div key={itemIndex} className="ui-repeater__row row">
    ...
    <button id="RemoveRepeaterItem" className="btn btn-light ui-btn--icon ui-repeater__remove-btn" onClick={() => removeItem(itemIndex)}>
  ...
  render() {
    ...
    return (
      <div className="ui-repeater">
        {this._buildList()}
        <button className="btn ui-btn ui-btn--icon ui-repeater__add-btn" onClick={() => this._addItem()}><i className="icon ion-md-add"></i></button>
      </div>
    );
  }
}
